                ...
                function renderLoop() {
                    if (videoElement.currentTime !== lastVideoTime) {
                        ...
                        if (results.landmarks && results.landmarks.length > 0) {
                            ...
                            // Pinch Distance
                            const distance = Math.sqrt(Math.pow((1 - thumbTip.x) - (1 - indexTip.x), 2) + Math.pow(thumbTip.y - indexTip.y, 2));
                            ...
                            if (distance < 0.05) {
                                ...
                                if (lastHandX !== null && lastHandY !== null) {
                                    currentDeltaX = (currentX - lastHandX) * 1000;
                                    currentDeltaY = (currentY - lastHandY) * 5;
                                } else {
                                    currentDeltaX = 0;
                                    currentDeltaY = 0;
                                }
                    ...
                    smoothedDeltaX = lerp(smoothedDeltaX, currentDeltaX, 0.1);
                    smoothedDeltaY = lerp(smoothedDeltaY, currentDeltaY, 0.1);

                    rotationZ += smoothedDeltaX * 0.2;
                    verticalKaificent += smoothedDeltaY * 0.001;
                    verticalKaificent = Math.max(0, Math.min(1, verticalKaificent));
